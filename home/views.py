...
def home(request):
    ...
    return render(request, 'home.html', context={'count': get_question_count()})


def review(request):
    # Lets the user know how far they are on the survey
    progress = int(get_complete_count(request.session.session_key))
...
class QuestionView(TemplateView):
    ...
    def get(self, request):
        ...
        if request.session.get('progress', None) is None:
            request.session['progress'] = 0
            request.session['last_choice'] = ()
        progress = get_complete_count(request.session.session_key)
        # Checks if there are any survey questions
        if check_complete(progress):
            return render(request, 'done.html')
        question, answers = get_question_answers(request.session.session_key)
